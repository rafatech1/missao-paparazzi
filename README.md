...
- Cada desafio aceita no máximo **5 fotos/vídeos**, exceto "Registre sua presença", que não tem limite (a exceção fica em `UNLIMITED_CHALLENGES`, tanto em `public/index.html` quanto em `api/_lib/media.js`). Depois de bater o limite, o desafio fica marcado como "completo" no site (botão desabilitado) e novos envios pra ele são recusados também no servidor — então dá pra contar com o limite mesmo se alguém tentar mandar por fora do site.
